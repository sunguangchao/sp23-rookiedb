...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        final long pageNum = newLeafNode.getPage().getPageNum();
        this.keys = leftKeys;
        this.rids = leftRids;
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        for (int i = 0; i < n; i++) {
            dataBoxes.add(DataBox.fromBytes(buf, metadata.getKeySchema()));
            recordIds.add(RecordId.fromBytes(buf));
        }
        return new LeafNode(metadata, bufferManager, page, dataBoxes, recordIds,
                rightSibling == -1L ? Optional.empty() : Optional.of(rightSibling), treeContext);
    }
