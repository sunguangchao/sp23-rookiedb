...
class InnerNode extends BPlusNode {
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        BPlusNode tmp = this;
        while (tmp.getClass() != LeafNode.class) {
            tmp = ((InnerNode) tmp).getChild(0);
        }
        return (LeafNode) tmp;
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Pair<DataBox, Long> dataBoxLongPair = put.get();
        this.keys.add(dataBoxLongPair.getFirst());
        this.children.add(dataBoxLongPair.getSecond());
        ...
        List<Long> rightChild = children.subList(order + 1, 2 * order + 2);
        InnerNode newNode = new InnerNode(metadata, bufferManager, rightKeys, rightChild, treeContext);

        this.keys = leftKeys;
        this.children = leftChild;
        sync();
        return Optional.of(new Pair<>(middleKey, newNode.getPage().getPageNum()));
    }
